...
export class ChannelManager extends EventEmitter3<ChannelEvents> {
    ...
    constructor(private readonly client: FC2Client) {
        super();
        this.on("live", (member) => {
    ...
    public async fetchMember<
        TChannel extends boolean,
        TProfile extends boolean
    >(
        id: string,
        options: { channel?: TChannel; profile?: TProfile }
        ...
        if (channel_data) {
            ...
            const channel: FC2Member<true, false>["channel"] = {
                ...
                loginRequired:
                    WATCHABILITY_LOOKUP[login_only as APIWatchability],
     ...
     */
    private async getChannelChanges(): Promise<{
        live: string[];
        offline: string[];
    }> {
        const [{ channel: safeChannels }, { channel: adultChannels }] =
            await Promise.all([this.fetchAll(false), this.fetchAll(true)]);
        ...
        const wentOffline = new Set(
            [...this.liveIds].filter((id) => !channels.has(id))
        );
    ...
    private async triggerEvents({
        ...
        const [liveChannels, offlineChannels] = await Promise.all([
            Promise.all(
                live.map((channelId) =>
                    this.fetchMember(channelId, {
                        channel: true,
                        profile: true,
                    })
                )
